  ...
  <div>
    ...
    <b-card no-body class="mb-0">
      ...
      <b-table ref="refUserListTable" class="position-relative" responsive primary-key="id" :items="listClients"
        empty-text="No patients found" :fields="fields" :filter="search" :per-page=0 id="my-table"
        :current-page="currentPage" show-empty>

        <template v-slot:cell(SettingValue)="{ item }">
          {{ "$" + item.SettingValue }}
        </template>
          ...
          <b-dropdown variant="link" no-caret :right="$store.state.appConfig.isRTL" transition="scale-transition"
            ...
            <b-list-group style="padding: 2px; margin-bottom: 2px" dense rounded>
              <router-link class="urlPagina"
                :to="{ name: 'form-prices-selfpaids', params: { id: item.id, item: item } }">
...
export default {
  ...
  data() {
    return {
      ...
      listClients: [],
      search: '',
      fields: ['SettingCode', { label: 'PACKAGE', key: 'SettingName' }, { label: 'PRICE', key: 'SettingValue' }, { label: 'FROM MILES', key: 'SettingDistance1' }, { label: 'TO MILES', key: 'SettingDistance2' }, { label: 'TRIP TYPE', key: 'SettingType' }, 'actions'],
      totalItems:0,
      pagination: {},
      lastPage:0,
      searchQuery:'',
  ...
  methods: {
    ...
    masterSearch(url=''){
      if(this.searchQuery == ''){
        ...
        if(url == ''){
          url = `admin/panel/GetAllPricesSelfPay/search/`+this.searchQuery
        }
    ...
    getClientes(url='') {
      if(url == ''){
        url = 'admin/panel/GetAllPricesSelfPay'
      }
      this.$swal({
        title: 'Please, wait...',
        didOpen: () => {
    ...
    deleteRecord(id) {
      this.$swal({
        ...
        if (result.value) { // <-- if confirmed
          ...
          this.$http.post(`admin/panel/priceselfpay/${id}/delete`)
            .then((res) => {
              this.getClientes();
              this.$swal({
                title: res.data.data,
