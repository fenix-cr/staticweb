    ...
    >
      ...
      >
        ...
        <template #cell(actions)="{ item }">
          ...
          >
            ...
            <b-list-group  v-if="hasPermissionRemoveUser" style="padding: 2px; margin-bottom: 2px" dense rounded>
                <b-list-group-item style="padding: 0" class="urlPagina" :ripple="false">
                  <b-list-group-item class="font-weight-bold"
                                     style="border: none; padding: 5px"
      ...
      <div class="mx-2 mb-2">
        <b-row>
          ...
          >
             <span class="text-muted">{{ pagination.from }} to {{ pagination.to }} of {{totalItems}} entries</span>
          </b-col>
          <!-- Pagination -->
          <b-col
              cols="12"
                    ...
                    <ul class="pagination justify-content-center">
                        <li class="page-item" :class="{ disabled: pagination.prev_page_url == null }">
                            <button type="button" class="page-link"
                                @click="getProviders(pagination.first_page_url)">&laquo;</button>
                        </li>
                        <li class="page-item" :class="{ disabled: pagination.prev_page_url == null }">
                            <button type="button" class="page-link"
                                @click="getProviders(pagination.prev_page_url)">Previous</button>
                        </li>
                        <li class="page-item" v-if="currentPage - 2 > 0">
                            <button type="button" class="page-link"
                                @click="getProviders(pagination.path + '?page=' + (currentPage - 2))">{{ currentPage
                                        - 2
                                }}</button>
                        </li>
                        <li class="page-item" v-if="pagination.prev_page_url">
                            <button type="button" class="page-link" @click="getProviders(pagination.prev_page_url)">{{
                                    currentPage - 1
                            }}</button>
                        </li>
                        <li class="page-item">
                        ...
                        <li class="page-item" v-if="pagination.next_page_url">
                            <button type="button" class="page-link" @click="getProviders(pagination.next_page_url)">{{
                                    currentPage + 1
                            }}</button>
                        </li>
                        ...
                        <li class="page-item" :class="{ disabled: pagination.next_page_url == null }">
                            <button type="button" class="page-link"
                                @click="getProviders(pagination.next_page_url)">Next</button>
                        </li>
                        <li class="page-item" :class="{ disabled: pagination.next_page_url == null }">
                            <button type="button" class="page-link"
                                @click="getProviders(pagination.last_page_url)">&raquo;</button>
                        </li>
                    </ul>
                </nav>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      detailDrivers: {},
      search: '',
      fields: ['company_name', 'phone', 'email', 'company_address', 'point_of_contact', 'account_number','actions'],
  ...
  methods: {
    ...
    masterSearch(url=''){
      if(this.searchQuery == ''){
        ...
        if(url == ''){
          url = `admin/panel/providerslist/search/`+this.searchQuery
        }
        
      this.$http.get(url).then((response) => {
        this.listProviders = response.data.data.data
    ...
    deleteProvider(id){
      ...
      this.$http.post(`/admin/panel/provider/${id}/delete`)
          .then((res) => {
            this.$swal({
